...
if page == "Zero-Trading":
    ...
    if market_df.empty:
        ...
        if advisor_live.empty:
            ...
            #
            # SELL has priority:
            #   C4 OR C5 OR C6 OR C7 -> Action = Sell
            #
            # BUY is considered only for non-SELL rows:
            #   NOT C4 AND NOT C5 AND NOT C6 AND NOT C7 AND C2 -> BUY candidate
            ...
            c7_decision = advisor_live.get(
                "C7", pd.Series(False, index=advisor_live.index)
            ).fillna(False).astype(bool)
            ...
            if advisor.empty:
                ...
                advisor["_InitTimeLatestRaw"] = advisor["Ticker"].astype(str).str.strip().str.upper().map(
                    zero_open_buy_times
                )
                ...
                if "C2" not in advisor.columns:
                    advisor["C2"] = False
                advisor["C2"] = advisor["C2"].fillna(False).astype(bool)
                for bool_column in ["C4", "C5", "C6", "C7"]:
                ...
                def _zero_row_style(row):
                    ...
                    if (
                        bool(raw.get("C6", False))
                        or bool(raw.get("C7", False))
                    ):
        ...
        st.caption(
            "Parameters: C2 is the global BUY breadth condition; C4 is the drop-from-peak condition; "
            "C5 is the static-price condition; C6 is the near-market-close weak-position exit; "
            "C7 is the near-market-close strong-profit exit. "
            "LastSelling is the estimated liquidity time to sell; LastTops is the current C2 ticker "
            "count and is '-' for Sell rows; LastClose2h is the approximately two-hour CloseB. "
            "Drop24h and Change24h use LastCollect-24h through LastCollect. DropInitTimeLatest and "
            "ChangeInitTimeLatest use the latest OPEN InitTime through LastCollect. "
            f"Drop values are bold above the C4/C5 movement threshold "
            f"({float(SELL_CONFIG.get('movement_percent', 1.1)):.2f}%). "
            f"Change values are bold below that threshold. WeakInitTimeLatest is the percentage "
            f"gain from the latest OPEN position's InitPrice to LastPrice. C6 is true only when "
            f"C4 and C5 are false, the market is open, regular market close is within the next "
            f"{float(SELL_CONFIG.get('c6_close_minutes', 30.0)):g} minutes, and WeakInitTimeLatest "
            f"is below the C6 minimum gain ({float(SELL_CONFIG.get('c6_min_gain_percent', 2.0)):.2f}%). "
            f"C7 uses the same reference price and close window, but is true when "
            f"WeakInitTimeLatest is above the C7 maximum gain "
            f"({float(SELL_CONFIG.get('c7_max_gain_percent', 5.0)):.2f}%). "
            f"WeakInitTimeLatest is bold when C6 or C7 is true."
        )

        if bool(SELL_CONFIG.get("details", True)):
            st.caption(
                "Details is enabled: C4 Check, C5 Check, C6 Check and C7 Check show the inputs "
                "used by each SELL condition. Disable Details in Settings to hide these diagnostic columns."
            )

        st.subheader("Steps to buy a ticker")
        st.markdown(
            "1. Is **Action == Buy** and **LastClose2h** high?\n"
        ...
        st.markdown(
            "1. Is **Action == Sell** and is the ticker open in the ZERO app?\n"
            "2. Was the ticker bought before **InitTimeLatest**?\n"
            "3. Is the ticker price drop larger than 2%, or is the ticker price change larger than 2%?\n"
    ...
    with st.form("rule_settings_form"):
        ...
        with details_col:
            new_details = st.checkbox(
                ...
                help=(
                    "Show C4 Check, C5 Check, C6 Check and C7 Check columns on Zero-Trading."
                ),
            )
        ...
        st.caption(
            ...
            "Details controls the Zero-Trading check columns."
        )
    ...
    # available. DiffSellPrice is a percentage, so the monetary P/L is:
    #
    #     InitPrice * DiffSellPrice / 100
    #
    ...
    if pd.isna(last_collected_time):
        ...
        # Build the five most recent Monday-Friday trading days,
        # newest first. The newest day may be incomplete.
        recent_days = []
        candidate_day = last_local.normalize()
        ...
        for day_start_local in recent_days:
            ...
            if not sim_results.empty and effective_end_utc > accounting_start_utc:
                ...
                running_open_assets = int(open_at_start_mask.sum())
                ...
                for _, open_event in open_events.iterrows():
                    running_open_assets += int(open_event["Delta"])
                    max_open_assets = max(
                        max_open_assets,
                        running_open_assets,
                    )
        ...
        st.caption(
            ...
            "end-of-day boundary. "
            "Profit is the sum of positive DiffSellPrice values of sold positions; "
            "Loss is the sum of negative DiffSellPrice values; Result = Profit + Loss. "
            "Profit, Loss and Result are percentage-point sums, matching DiffSellPrice "
            "on Sim-Trading."
        )
